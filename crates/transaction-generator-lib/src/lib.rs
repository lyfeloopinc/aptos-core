...
use publishing::{entry_point_trait::{EntryPointTrait, PreBuiltPackages}, publish_util::PackageHandler};
    ...
    for transaction_mix in transaction_mix_per_phase {
        ...
        for (transaction_type, weight) in transaction_mix {
            let txn_generator_creator: Box<dyn TransactionGeneratorCreator> = match transaction_type
            {
                ...
                TransactionType::PublishPackage { use_account_pool , pre_built, package_name } => wrap_accounts_pool(
                    Box::new(PublishPackageCreator::new(txn_factory.clone(), PackageHandler::new(pre_built, &package_name))),
                    use_account_pool,
                    &accounts_pool,
                ),
